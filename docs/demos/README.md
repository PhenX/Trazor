...
| [`gradient-fills`](gradient-fills.ts)         | Smooth ramps traced with gradient detection off (posterized bands) vs. on (one `<linearGradient>`/`<radialGradient>`); a flat control stays byte-identical. |
