...
| [`max-colors-cap`](max-colors-cap.ts)                   | The palette budget as a hard cap under region growing: a cartoon panel grown into regions at Max colors 24, 8 and 4 — the count never exceeds the budget, and each cut folds the pair whose union adds the least squared color error (a small accent first, a fill and its shading tone next, distinct large hues last). |
| [`region-size-aware-merge`](region-size-aware-merge.ts) | Size-aware region merging (SRM): a flat sunset of close colors washed into one by a flat merge threshold vs. kept apart by a tolerance that shrinks as regions grow — small specks still fold, large fields survive.                                                                                                     |
| [`precision`](precision.ts)                             | SVG coordinate precision — the one-decimal default against two decimals on the same trace: renders at 1×, a magnified crop where the two differ most, the amplified difference, both outlines overlaid, one path command at each precision, and bytes / gzip / node counts. Takes `--data <dir>` for your own images.    |
| [`lineart-overink`](lineart-overink.ts)                 | A faint tonal line-art scan traced as black & white (a threshold thickens every gray stroke and floods the hatching solid black) vs. the recommender's grayscale tonal route, which keeps each stroke at its true darkness — the fix for over-inked scanned drawings.                                                    |
