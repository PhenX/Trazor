...
| [`palette-key-colors`](palette-key-colors.ts)           | The global-palette front-end on compressed flat art (a cartoon JPEG-encoded with chroma subsampling): flat-region seeds and the thin-variant merge off vs. on — label maps, palettes, and the key-color / boundary indicators against the clean scene.                                                                |
