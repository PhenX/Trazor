...
- **File:** `_source-sunset.svg` (kept here for provenance / regeneration)
  - **Title:** _Sunset over water_ — an original gradient scene (linear sky,
    radial sun, gradient sea) authored for this project to exercise gradient
    detection.
...
| `gradients-before.png` / `gradients-after.png`     | Gradient fills             | `_source-sunset.svg`, Photo profile + `layering: 'cutout'`, `gradients: false` vs `true`                     |
