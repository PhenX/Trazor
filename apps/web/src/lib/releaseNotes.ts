...
export const RELEASE_NOTES: readonly ReleaseNote[] = [
  {
    ...
    items: [
      'Color and grayscale traces can now paint smooth color ramps — skies, soft shading, backgrounds, spotlights — with a single SVG gradient instead of a stack of posterized bands, so the result looks smoother and uses fewer shapes. Both straight (linear) and circular (radial) ramps are recognized. Turn on "Gradient fills" in the palette settings; it is on by default in the Illustration and Photo / Poster presets.',
