...
1. **Public API** — everything re-exported from `src/index.ts`: `traceMask`, `traceLabelMap`, `traceCenterline` and
   their option/result types, plus the lower-level `decomposeMask`, `shapesFromPaths`, `optimalPolyline`,
   `fitOpenPolyline`, `simplifyOpen`, `reverseCommands`, `refineRingToField`. The engine calls these; changing a
   signature is a cross-package change — update
